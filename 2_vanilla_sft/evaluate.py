"""
Evaluate vanilla SFT model on GSM8k test set.
GPU-optimized with batch processing.

Usage:
    python evaluate.py --model results/model --batch_size 64
    python evaluate.py --save_results  # Save detailed results to JSON
"""
...
def sft_prompt(question: str) -> str:
    """SFT prompt format (no CoT instruction)."""
    return f"Q: {question}\nA: "
...
def main():
    ...
    parser.add_argument("--model", default="results/model")
    ...
    for batch_start in range(0, total, args.batch_size):
        ...
        batch_prompts = [sft_prompt(q) for q in batch_questions]
    ...
    if args.save_results:
        ...
        filename = f"results/vanilla_sft_{model_name}_{args.split}_{timestamp}.json"
